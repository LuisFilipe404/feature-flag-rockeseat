...
var jsonLogicEngine = require('json-logic-engine');
...
function semVer(data, context) {
    ...
    const semVer1 = semver.parse(args[0]);
    const semVer2 = semver.parse(args[2]);
    if (!semVer1 || !semVer2) {
        logger.debug(`Invalid ${semVerRule} configuration: Unable to parse semver`);
        return false;
    }
    const operator = String(args[1]);
    const result = semver.compare(semVer1, semVer2);
    switch (operator) {
        case '=':
            return result == 0;
        case '!=':
...
class Targeting {
    constructor(logic, logger) {
        this.logger = logger;
        const engine = new jsonLogicEngine.LogicEngine();
...
class FeatureFlag {
    constructor(key, flag, logger) {
        ...
        this._hash = objectHash.sha1(flag);
    ...
    evaluate(evalCtx, logger = this.logger) {
        ...
        if (this._targetingParseErrorMessage) {
            return {
                ...
                errorCode: core.ErrorCode.PARSE_ERROR,
        ...
        if (!this._targeting) {
            ...
            reason = core.StandardResolutionReasons.STATIC;
        }
        else {
    ...
    validateStructure() {
        ...
        if (this._state !== 'ENABLED' && this._state !== 'DISABLED') {
            throw new core.ParseError(`Invalid flag state: ${JSON.stringify(this._state, undefined, 2)}`);
        }
        if (this._defaultVariant === undefined) {
            // this can be falsy, and int, etc...
            throw new core.ParseError(`Invalid flag defaultVariant: ${JSON.stringify(this._defaultVariant, undefined, 2)}`);
        }
        if (!this._variants.has(this._defaultVariant)) {
            throw new core.ParseError(`Default variant ${this._defaultVariant} missing from variants ${JSON.stringify(this._variants, undefined, 2)}`);
        }
    }
}
...
function parse(flagConfig, strictValidation, logger) {
    ...
    try {
        ...
        if (!isValid) {
            ...
            if (strictValidation) {
                throw new core.ParseError(message);
            }
            else {
        ...
        if (err instanceof core.ParseError) {
            throw err;
        }
        throw new core.ParseError(errorMessages, { cause: err });
    }
}
...
class FlagdCore {
    constructor(storage, logger) {
        this._logger = logger ? new core.SafeLogger(logger) : new core.DefaultLogger();
    ...
    resolve(type, flagKey, defaultValue, evalCtx = {}, logger = this._logger) {
        ...
        if (!flag) {
            return {
                value: defaultValue,
                reason: core.StandardResolutionReasons.ERROR,
                errorCode: core.ErrorCode.FLAG_NOT_FOUND,
